...
- (void)uploadString:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback;
...
RCT_EXPORT_METHOD(upload:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback)
{
  ...
  NSString *payload = input[@"payload"];
  if(!([payload length] != 0 ^ [filePath length] != 0)){
    callback(@[RCTMakeError(@"Either url or file must be specified, not both", nil, nil)]);
  }
  else if([payload length] > 0){
    [self uploadString:input callback:callback];
  }
...
- (void)uploadUri:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback
{
  NSString *dataUrlString = input[@"file"];
...
- (void)uploadString:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback
{
  ...
  [self sendData:data withOptions:input callback:callback];
}
